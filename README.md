...
「请求 / Token 趋势」与「供应商统计」：按日/时/月维度展示 Token 用量走势，并按供应商汇总请求量、成功率、延迟与缓存命中率（如 OpenCode GO 套餐可达 99%）。

> 提示：面板同时提供「请求日志」「Token 统计」「趋势图表」「供应商健康」「API Key 管理」等标签页，全部通过内嵌 WebView 或浏览器访问 `http://127.0.0.1:8787/admin` 即可使用。
