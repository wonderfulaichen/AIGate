...
- **「Token 活动」重做为时段 × 星期热力图**：这块前后错了两次，记下完整经过 ——（a）原实现是「日历热力图」，代码注释里明确写着**与所选粒度解耦**（固定按天聚合），于是选「小时(24)」只有 1 列、选「天(30)」只有 5 列，一小块方格浮在 1149px 宽的面板左侧像没收好的残片，**上方的时间范围选择器等于成了摆设**（选了 30 天却只画 5 列）；（b）我随后误判成「格子太大/太空」，把方格缩小并加日历标签，反而更像残片；（c）第二次又擅自把热力图换成了柱状图 —— 那不是热力图，方向错了。**根因是矩阵维度选错了**：只要行是「周」、列是「日」，30 天就必然是 5 列，无论怎么排都填不满宽度。现改为 **24 列（0-23 时）× 7 行（周一~周日）**：格子用 `aspect-ratio:1` 保持正方形并按 `1fr` 均分宽度，因此**任何范围下都是一整片 44×44 的满方格**（实测 168 格 = 24×7，铺满 1149px）；颜色深浅表示该（星期, 时段）的 Token 用量，浅底表示该时段没有请求；顶部标注小时刻度（每 3 小时一个）。时间范围选择器由此重新生效（决定聚合哪一段时段数据，逐时数据上限 29d —— 更早的日期 rollup 只有天粒度）。逐日细节不在本图，由下方 Token 趋势图承担。图例同步由「输入/输出」改为强度刻度（较少 → 较多）。
