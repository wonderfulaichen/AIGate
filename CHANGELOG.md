...
- **「Token 活动」改为按桶堆叠柱图（跟随所选范围）**：原实现是「日历热力图」，且代码注释里明确写着**与所选粒度解耦**（固定按天聚合）—— 于是选「小时(24)」只有 1 列、选「天(30)」只有 5 列，一小块方格浮在宽面板左侧像没收好的残片，**上方的时间范围选择器等于成了摆设**（选了 30 天却只画 5 列）。现改为按当前范围的**桶**出柱：24h→24 列 / 30d→30 列 / 12m→12 列，铺满面板宽度；每列按输入/输出分色堆叠，柱高按「该桶 token ÷ 全范围峰值」归一；空桶画一条细底线占位（「那段时间没有请求」要看得出来，但不必给出零高柱）；列多时标签自动抽稀。取数也从「单独按天拉一次」改为与趋势图同源（同一粒度/范围的 trends），消除两处口径不一致的根源。此形态与参考项目「每日编码活跃度」一致（`dayStack`/`dayCol`/`daySeg`/`dayEmptyDot`：列数跟随整个范围、空日同样占一列）。
