...
### 新增
- **「降级可回取」：留存被转发优化移除的原文（设置页开关，新模块 `recall.rs`）**：借鉴 RTK 的 recall store 设计（`[full output: rtk recall <hash>]`）。此前剥离推理链（`m.remove` 直接删）、截断工具输出都是**有损且不可逆**的——面板只有"省了多少 token"的计数，**无法回答"这次到底删了什么"**，用户怀疑模型变笨时只能关开关，当次请求无从复查。现留存被移除的原文到 `data/recall/`（内容寻址去重 + 单项 256KB 上限 + FIFO 200 条），设置页可查看列表/取回原文/清空，并统计**回取次数**（RTK `rtk gain --recalls` 的思路：用数据判断截断是否伤害了实际使用）。**默认关闭**（留存内容是推理链与工具输出明文，落盘属隐私取舍，开启时弹确认）；取回接口对 id 做十六进制白名单校验防路径穿越；UTF-8 截断边界安全。端到端实测：剥离后上游收到 `marker_kept=False`（确实没发）而本地留存可原样取回。
- **常态截断超长工具输出（设置页可配阈值，`AIGATE_TOOL_OUTPUT_MAX_BYTES`，默认 0=关）**：此前 `truncate_tool_outputs`（32KB）只藏在 `emergency_shrink_body` 里、且仅当供应商配了 `max_request_body_bytes` 才触发——**7 个供应商一个都没配，该功能从未生效过**，而实测 6.5% 请求体 >1MB（p99 2.8MB），单条 tool 消息可含整个文件内容、每轮重发属"注水"。现提升为常态开关（默认关，开启需确认有损），截断点通过回调把被移除的原文一并送进 recall 留存。端到端实测：20KB 工具输出截至 4KB 上限，上游未收到全文，被截 15916 字节已留存。
- **静默降级埋点（分析页省量审计面板，`GET /admin/api/degradations`）**：参照 RTK 的 `parse_failures` 表。上游错误响应体无法按协议翻译时（如网关 502 HTML 页），原路径用 `.unwrap_or(err_body)` **静默**放行——客户端看到上游原文且无人知道翻译失败过。现计数 + warn 日志，分析页展示「静默降级次数」（含分项与口径说明）。**只记真实可达的降级**：曾尝试给"请求体解析失败"也加计数，端到端测试证明该分支不可达（入口 `parse_model` 已完整解析，失败直接 400），已删除该枚举并留注释说明原因——永远为 0 的计数只会误导排查方向。端到端实测：anthropic 模型 + 上游 502 HTML → `upstream_error_parse_failed` 0→1。
- **模型价格面板显示「官方参考价」（不自动写入）**：models.dev 本就带 `cost` 字段（USD/百万 tokens），而 AIGate 一直在拉这个数据源却只读了上下文/视觉等字段。现价格面板底部展示参考价，按面板显示币种汇率换算后标明来源。**刻意不自动填入 `price`**，因为三方对照后发现它对你的官方供应商**不可用**：①**缺时段维度** —— DeepSeek 官方是峰谷双价（空闲价 = 高峰价一半，工作日 9-12/14-18 为高峰），共 6 个价目栏位，而 models.dev 只有单一价；②**币种与数值都对不上** —— models.dev 是 USD，按 7.2 换算后 `deepseek-flash` 得 1.08/4.32/0.02 元，而官方是 2.0/8.0/0.04（高峰）或 1.0/4.0/0.02（空闲），**两档都不吻合**；`deepseek-v4-pro` 偏差达 44%。已手工配准的 DeepSeek 模型与官方定价页逐项一致，自动填入反而会改错。
- **参考价按「同名供应商」严格匹配**：models.dev 收录 200+ provider，**同一模型被几十家重复收录且报价差异极大** —— 实测 `deepseek-v4-flash` 有 **34 个 provider** 贡献同一归一化键（官方 0.15/0.6、azure 0.19/0.51、`llmgateway` 0.05/0.1，而 `alibaba-token-plan`/`sensenova` 等订阅制直接报 **0/0**）。原先扁平的 `HashMap<模型名, 元信息>` 会让后者覆盖前者（实测 DeepSeek 因此取到 0），故参考价改为独立的 `(provider, 归一化模型名)` 索引；**匹配不到同名供应商就不显示**，不做跨家回退（拿别家的价当"参考"只会误导）。实测：`deepseek` 取到 0.15、`azure` 取到它自己的 0.19，两者互不覆盖；未知供应商返回空。
- **「剥离带工具调用的推理链」改为按协议控制（设置页）**：逐模型勾选要求用户逐个确认，而**判据其实是协议**——供应商只是换个 endpoint（同一个供应商常同时提供三种协议），协议才决定剥离是否安全。现设置页「性能与优化」新增按协议开关，与逐模型勾选形成两级：`Chat`（`/v1/chat/completions`，`tool_calls` 是结构化数组，剥掉不破坏配对，**默认开**）、`Responses`（`/v1/responses`，结构上可剥但上游是否校验 item 序列未实测，**默认关**，可在面板试开）。`Anthropic`（`/v1/messages`）**不给开关**——该协议要求 `thinking` 块与 `tool_use` 并存，剥掉必然 400，代理层硬性跳过（优先于模型级配置，避免误勾制造必然失败的请求）。优先级：模型级显式勾选 > 协议白名单。运行时热切换、重启回到环境变量/默认值；新增 `STRIP_TOOLCALL_ON_CHAT` / `STRIP_TOOLCALL_ON_RESPONSES` 环境变量。
...
### 供应商与路由
- **模型中转 ID 会跨供应商撞车，导致改配置"没反应"**（正确性）：拉取模型时，若上游返回的 ID 自带斜杠（OpenRouter / commandcodeAI 等用「作者/模型」命名，如 `deepseek/deepseek-v4-flash`），旧代码**不再加中转供应商前缀**，原样当作中转 ID。而路由表是 `HashMap<中转ID, 路由>` —— 一个 ID 只能有一条路由，**后加载的供应商静默覆盖前者**。实测你的配置：**25 个 ID 重复、28 个条目永不生效**（`deepseek/deepseek-v4-flash` 同属 deepseek / Openrouter / commandcodeAI 三家），日志佐证 `deepseek/deepseek-v4.1-flash` 共 4117 次请求**全部走 commandcodeAI**，Openrouter 那条一次都没用过；被覆盖条目在面板里**可见、可改**（改协议/思考档位/价格），却**毫无效果也无任何提示**，还有 **2 条已配价格的条目永远不会被结算**。现中转 ID **一律**生成为 `供应商/上游模型ID`（上游自带斜杠也照加，幂等不会重复叠加），实测按新规则重生成后冲突数 **25 → 0**。前端拉取与 CSV 导入、后端 `add_models` 三处规则统一。
- **重复中转 ID 现在会被明确指出，但不会阻断保存**：保存配置时检测跨供应商的同名 ID 并在模型 ID 下方给出橙色警示（指名冲突的另一方），保存成功提示也附注重复条数。**只告警不拦截**——存量配置本就有历史重复，硬拦会让用户连无关字段都存不下去。新增 `transit_model_id`（含幂等与冲突消除测试）与反例对照测试。

### 界面与交互
- **「响应缓存」在设置页被画成了与「性能与优化」同级的独立分区**（画面）：它是一个**实验性子功能**（面板里已带「实验功能」徽标），却用与外层分区完全相同的 `.st` 标题形态 + 独立的 `.pnl` 面板呈现，视觉上等于告诉用户「这是和性能优化并列的另一块设置」，而它与上方的历史推理链瘦身 / 历史裁剪 / 断流续写 / 连接参数本该是一组。现降级为**组内子卡头**：并入「性能与优化」面板内部（新增 `.sub-h`，11px 大写弱化标题 + 上边框分隔），不再占独立分区层级。实测缓存块父链为 `sub-h → pnl → page → main`，已位于该面板内。
...
- **「清空所有记录」确认后到完成之间毫无反馈，按钮也不禁用**（交互）：第 8.7 节给它补了二次确认，但点下「确定」之后、请求返回之前，界面**没有任何进行中状态**，按钮仍可点。而清理会重写日志文件并删除日级账本，不是瞬时操作 —— 用户可能连点两次（第二次已无数据可清），或因毫无反馈而以为没生效、切页或重复操作。现引入进行中标记：期间按钮 `:disabled` 且文案变为「保存中…」，并直接忽略再次点击（连确认框都不再弹）。**同时补齐失败路径**：非 2xx 与抛错都给出错误提示，并**在 `finally` 里复位标记**（否则按钮会永久禁用）；失败时保留原数据，不谎报"已清空"。
