...
- **6 个纯图标按钮没有任何说明，其中 2 个是删除操作**（提示 / 交互）：侧栏折叠按钮与 5 个 `✕` 按钮此前**既无 `title` 也无 `aria-label`** —— 鼠标用户必须逐一点开才知道哪个是"删除供应商"、哪个只是"关闭面板"；两个删除按钮（`deleteProvider` / `deleteModel`）在外观上与其他 `✕` 完全一样（都是灰色小叉，只有颜色区分）。虽然删除已有二次确认兜底，但按钮本身不说明自己做什么，等于让用户凭猜测点击。现全部补上可访问名：侧栏用「收起 / 展开侧栏」、两个删除用「删除」、移除时段用「移除该时段」、两个关闭用「关闭」（新增 `close_label` / `remove_peak_window` / `toggle_sidebar` 三键，中英各一；`delete` 复用现有键）。**实测确认**：侧栏 `title`/`aria-label` = 「收起 / 展开侧栏」，页面上各 `✕` = 「移除该时段」/「关闭」，纯图标按钮中无可访问名的数量由 **10 个降到 4 个** —— 剩余 4 个是 `.tgl` 开关，旁边都有可见文字标签，不属缺陷。
- **设置页「连接参数」一行被挤断，行高是其它行的 2.4 倍**（画面）：该行有三个控件（流空闲超时 / 瞬态失败重试次数 / 重试退避基数），实测右侧需要 347px 而只剩 341px —— **差 6px**，第三个控件被挤到第二行，行高从 68px 涨到 **161px**，而同一页其余 12 行都是 55–70px。根因：`.row-d`（说明文字）只有 `max-width:560px` 而没有收缩下限，长说明会占满可用宽度、把右侧控件组压到换行。现给 `.set-row` 的直接子元素加约束：左侧 `flex:1 1 auto; min-width:0`（可收缩）、右侧 `flex:0 0 auto`（保住宽度）。实测修复后该行 88px、三控件回到同一行，其余 12 行高度不变。
- **引导文案把界面元素的位置说反了**（引导）：零数据时概览的引导语写着「将**上方**地址填入编程工具的 API 地址」，而地址显示在这句话的**下方**（提示在 L483、地址框在 L484）—— 用户照字面找"上方"会先看到标题与副标题，找不到地址。现改为「下方地址」（英文 `address above` → `address below`），与实际渲染顺序一致。同批修掉设置页 `currency_hint` 的同类耦合：「按**上方**汇率换算」而汇率输入其实在**同一行的右侧**（`.set-row` 是 `justify-content:space-between` 的 flex 布局），改为「按**所选**汇率换算」/ `using the selected rate`，不再把文案绑死在某个方位上（布局若调整，文案不会随之失效）。**注**：审计面板的「省量见上方…」经核实方位正确（被引用项确实在上方），未改动。
- **概览「TOKEN 总量」卡把正下方三张卡的数字又写了一遍，且删除后导致三张卡折线错位**（画面）：主卡提示原为「658.0M 输入 · 3.9M 输出」，而**紧挨着的下一行**就是三张独立卡片「输入 Tokens 658.0M / 输出 Tokens 3.9M / KV Cache 命中 97.1%」—— 同样的数字相隔两厘米说两遍，且下方那行信息更全（多一个缓存命中）。**实测确认后**删掉主卡那句重复提示。**同时发现并修掉一个副作用**：删掉提示后，三张卡内部元素变成 label → value → mini-spark（另两张多一行 hint），第 3 张卡的迷你折线**上移 22px**（实测 `sparkTop` 307 vs 320），三张卡的折线不在同一水平线上。现让 `.ov-summary-card` 改为纵向 flex、`.mini-spark` 用 `margin-top:auto` 贴底，实测三张卡折线 `sparkTop` 均为 320、卡片等高 143。随删除失效的 `tokens_input_output` 键（中英各一）一并清理。
- **分析页同一屏把「天 粒度 · UTC+8」说了两遍**（提示）：页头右侧一处，Token 趋势面板标题下又一处（`trendContextLabel`），两者输出完全相同。现让趋势面板那处**只在有增量信息时才出现** —— 即图表做了自适应裁剪、展示区间不同于整个窗口时，显示「实际展示 09/11 → 09/16」；无裁剪时返回空串并 `x-show` 不占位（否则 `margin-top:4px` 仍会留一条空白）。粒度与时区由页头统一承担，信息不丢、只是不再重复。新增 `trend_shown_range` 键（中英各一），替换原先复用 `stats_context` 的写法。
- **移除页面底部的红色调试条**（画面）：页面内置了一个开发期调试面板 —— 它劫持 `console.warn` / `console.error`、监听 `error` 与 `unhandledrejection`，把捕获到的报错以红底横条**常驻显示在窗口底部**（`#dbgbar`，`position:fixed;bottom:0;z-index:99999`，每 600ms 刷新一次）。它自成一个闭环（写入 `window.__errs` → 轮询 → 显示），既无开关、也无别处读取，属开发排查时的残留。两个实际危害：一是横条**横跨整个窗口底部遮挡内容**；二是把**内部报错原样摊给用户看** —— 实测线上实例底部就常驻着 `CERR: JSON error: missing field 'cmd' at line 1 column 113` 这类上游/客户端 JSON 细节。现已连同其 CSS、DOM 节点与轮询一并删除（共 5 处、-15 行），未保留任何开关（如后续需要调试，用浏览器 DevTools 即可）。
- **概览 hero 的请求提示把主数字重复了一遍**（提示）：主数字已经是「请求总数」，而下方提示渲染成「错误 70 · **总计 2619** · 今日 22」—— 其中 2619 与主数字完全相同，等于同屏把同一个数说了两遍、白占提示行三分之一。现提示只补主数字没有的信息：`错误 N`（有今日数据时附 `· 今日 M`）。原 `errors_total` 键（`错误 {0} · 总计 {1}`）随之失去用处，改名并简化为 `errors_only`（`错误 {0}` / `{0} errors`）。
- **「清空所有记录」的确认框建议用户"先导出 JSON"留档，但导出只能覆盖 23% 的数据**：这是上一条确认框引入时的措辞失当 —— 导出走的是 `this.logs`（`GET /admin/api/logs`，受 `LOG_CAPACITY=1000` 限制），而删除清掉的是磁盘上的全部 + 日级账本，**且没有全量导出接口**。实测线上：磁盘 4290 条时导出仅 1000 条（覆盖率 **23.3%**），导出内容的时间跨度只有约 3 小时（全量是 6 天），2748 条价格快照中仅 1000 条可得。按原措辞操作的用户会以为已备份，实际丢弃 77% 的历史。现改为**如实告知**：「『导出 JSON』只含当前已加载的最近 N 条，无法保全全部历史」，N 由实时 `logs.length` 传入（避免硬编码与 `LOG_CAPACITY` 脱节）。
- **记录页的「错误请求 / 成功率」与概览的数字对不上，两者却都声称是「窗口」**：`GET /admin/api/logs` 受 `LOG_CAPACITY`（1000）限制只返回最近 1000 条，而 `/admin/api/stats`（概览用）统计的是后端整个缓冲区 —— 记录页拿前者算 `recordErrorCount` / `recordSuccessRate`，概览拿后者，于是同屏出现两个「窗口错误数」。实测线上：概览「错误 70」vs 记录页「错误 15」，**差 55 条**；成功率同理失真。现把记录页首个统计卡的标签由「当前窗口请求」改为「**已加载请求**」（中英同步），并在统计卡下方补一行口径披露，写明「只覆盖当前已加载的最近 N 条（内存缓冲区上限），与概览的窗口累计口径不同，完整窗口数据请看概览或分析页」，N 取实时 `logs.length`。**只改标签与说明，未改任何计算** —— 记录页需要的就是"当前这批"的统计（它逐条列出这 1000 条），错的只是"窗口"这个措辞。
